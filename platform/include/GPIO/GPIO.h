...
GPIO_Status_t GPIO_ConfigPin(uint8_t channel, GPIO_Direction_t direction, bool pullupEnable);

// Write value to a pin
GPIO_Status_t GPIO_WritePin(uint8_t channel, GPIO_Value_t value);

// Read value from a pin
GPIO_Status_t GPIO_ReadPin(uint8_t channel, GPIO_Value_t *value);

// Toggle output pin
GPIO_Status_t GPIO_TogglePin(uint8_t channel);
